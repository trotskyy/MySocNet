...
namespace DalTest
{
    ...
    class DalTestsHelper
    {
        public static void WriteTestDataToDatabase(MySocNetContext dbContext)
        {
            //all Threads, Messages, Posts, Users and UserRelations
            List<User> users;
            List<ConvThread> threads;
            List<Message> messages;
            List<Post> posts;
            List<UsersRelation> userRelations;
            ...
            //all Threads, Messages, Posts, Users and UserRelations
            users = new List<User>() { u1, u2, u3, u4, u5, u6, u7, u8, u9, u10 };
            messages = new List<Message>() { msg1, msg2, msg3, msg4, msg5, msg6, msg7, msg8, msg9 };
            posts = new List<Post>() { post01, post02, post03, post04, post05, post06, post07, post08, post09, post10, post1, post2, post3, post4, post5, post6, post7, post8, post9 };
            threads = new List<ConvThread>() { u1Thread, u2Thread, u3Thread, u4Thread, u5Thread, u6Thread, u7Thread, u8Thread, u9Thread, u10Thread, electroThread, hcThread, oldMuzThread, skateboardingThread };
            userRelations = new List<UsersRelation>() { ur1, ur2, ur3, ur4, ur6, ur7, ur8, ur9, ur10, ur11, ur12, ur13 };

            //adding to DB
            dbContext.Set<User>().AddRange(users);
            dbContext.Set<Message>().AddRange(messages);
            dbContext.Set<Post>().AddRange(posts);
            dbContext.Set<ConvThread>().AddRange(threads);
            dbContext.Set<UsersRelation>().AddRange(userRelations);
